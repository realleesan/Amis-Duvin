...
<div class="space-y-6">
  ...
  <!-- Bulk Action Bar -->
  <div id="bulkTrashBar" class="hidden items-center justify-between p-3.5 rounded-sm shadow-md mb-4" style="border: 1px solid rgba(178, 2, 37, 0.35);">
    <div class="flex items-center gap-2 text-xs text-foreground font-medium">
      ...
      <span>Đã chọn <strong id="bulkTrashCount" class="text-[var(--gold)] font-mono font-bold">0</strong> mục</span>
    </div>
    <div class="flex items-center gap-2">
      <button type="button" onclick="submitBulkTrash('restore')" class="px-3 py-1.5 rounded text-xs font-medium bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/25 transition-colors flex items-center gap-1.5">
      ...
      <button type="button" onclick="submitBulkTrash('force-delete')" class="px-3 py-1.5 rounded text-xs font-medium bg-rose-500/15 text-rose-400 border border-rose-500/30 hover:bg-rose-500/25 transition-colors flex items-center gap-1.5">
  ...
  <!-- Trash Data Table Form -->
  <form id="bulkTrashForm" method="POST" action="<?= admin_url('trash/bulk-restore') ?>">
          ...
          <thead>
            <tr class="bg-muted/40 text-[11px] uppercase tracking-wider text-muted-foreground border-b border-border/40 whitespace-nowrap">
              <th class="p-4 w-10 text-center">
                <input type="checkbox" id="selectAllTrash" onclick="toggleSelectAllTrash(this)" class="rounded border-border cursor-pointer">
              </th>
              <?php if ($type === 'bookings'): ?>
                ...
                <th class="p-4 w-36">Ngày xóa</th>
                <th class="p-4 w-52 text-right">Thao tác</th>
              ...
              <?php elseif ($type === 'users'): ?>
                ...
                <th class="p-4 w-28">Vai trò</th>
                <th class="p-4 w-36">Ngày xóa</th>
                <th class="p-4 w-52 text-right">Thao tác</th>
              <?php elseif ($type === 'notifications'): ?>
                <th class="p-4 w-28">Loại</th>
                <th class="p-4 min-w-[250px]">Tiêu đề / Nội dung</th>
                <th class="p-4 min-w-[150px]">Thực hiện bởi</th>
                <th class="p-4 w-36">Ngày xóa</th>
                <th class="p-4 w-52 text-right">Thao tác</th>
              <?php endif; ?>
            </tr>
              ...
              <?php foreach ($items as $item): ?>
                <tr class="hover:bg-muted/20 transition-colors">
                  <td class="p-4 text-center">
                    <input type="checkbox" name="ids[]" value="<?= $item['id'] ?>" class="trash-cb rounded border-border cursor-pointer" onchange="updateBulkTrashBar()">
                  </td>
                  ...
                  <td class="p-4 text-right whitespace-nowrap">
                    <div class="inline-flex items-center gap-2">
                      <button type="button" onclick="singleTrashAction('restore', <?= $item['id'] ?>)" class="px-2.5 py-1 rounded text-xs font-medium bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-500/25 transition-colors inline-flex items-center gap-1">
                      ...
                      <button type="button" onclick="singleTrashAction('force-delete', <?= $item['id'] ?>)" class="px-2.5 py-1 rounded text-xs font-medium bg-rose-500/15 text-rose-400 border border-rose-500/30 hover:bg-rose-500/25 transition-colors inline-flex items-center gap-1">
...
function updateBulkTrashBar() {
  const checked = document.querySelectorAll('.trash-cb:checked');
  const all = document.querySelectorAll('.trash-cb');
  const bar = document.getElementById('bulkTrashBar');
  const countEl = document.getElementById('bulkTrashCount');
...
function submitBulkTrash(action) {
  const form = document.getElementById('bulkTrashForm');
  if (!form) return;
  if (action === 'force-delete') {
    if (!confirm('Bạn có chắc chắn muốn XÓA VĨNH VIỄN các mục đã chọn? Thao tác này không thể hoàn tác!')) return;
    form.action = '<?= admin_url('trash/bulk-force-delete') ?>';
  } else {
    form.action = '<?= admin_url('trash/bulk-restore') ?>';
  }
  form.submit();
}

function singleTrashAction(action, id) {
  const form = document.createElement('form');
  form.method = 'POST';
  form.action = action === 'force-delete' ? '<?= admin_url('trash/force-delete') ?>' : '<?= admin_url('trash/restore') ?>';
  
  if (action === 'force-delete' && !confirm('Bạn có chắc chắn muốn xóa vĩnh viễn mục này?')) return;
