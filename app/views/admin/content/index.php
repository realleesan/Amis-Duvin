...
<div class="space-y-8">
  ...
  <div class="space-y-8">
    ...
    <div class="rounded-sm border border-border bg-card p-6 sm:p-8 space-y-6">
      ...
      <form action="/admin/content/hero" method="POST" class="space-y-5">
        <div class="grid sm:grid-cols-2 gap-5">
          <div>
            ...
            <input type="text" name="tagline" value="<?= htmlspecialchars($hero['tagline'] ?? '') ?>" class="w-full bg-background border border-border text-foreground px-4 py-3 rounded-sm text-sm font-medium">
          </div>

          <div>
            <label class="block text-xs uppercase tracking-widest text-muted-foreground mb-2">Tiêu đề chính dòng 1</label>
            <input type="text" name="title_main" value="<?= htmlspecialchars($hero['title_main'] ?? '') ?>" class="w-full bg-background border border-border text-foreground px-4 py-3 rounded-sm text-sm font-medium">
          </div>
        </div>
        ...
        <div class="grid sm:grid-cols-2 gap-5">
          <div>
            ...
            <input type="text" name="title_sub" value="<?= htmlspecialchars($hero['title_sub'] ?? '') ?>" class="w-full bg-background border border-border text-foreground px-4 py-3 rounded-sm text-sm font-medium">
          </div>

          <div>
            <label class="block text-xs uppercase tracking-widest text-muted-foreground mb-2">Đường dẫn Ảnh nền (Banner URL)</label>
            <input type="url" name="bg_image" value="<?= htmlspecialchars($hero['bg_image'] ?? '') ?>" class="w-full bg-background border border-border text-foreground px-4 py-3 rounded-sm text-sm font-medium">
          </div>
        </div>

        <div>
          <label class="block text-xs uppercase tracking-widest text-muted-foreground mb-2">Đoạn văn mô tả tóm tắt</label>
          <textarea name="description" rows="3" class="w-full bg-background border border-border text-foreground p-3 rounded-sm text-sm leading-relaxed"><?= htmlspecialchars($hero['description'] ?? '') ?></textarea>
        </div>

        <div class="pt-2">
          <button type="submit" class="btn-wine px-6 py-3 rounded-sm text-xs uppercase tracking-widest font-medium">
      ...
      <form action="/admin/content/service-intro" method="POST" class="space-y-5">
        <div class="grid sm:grid-cols-3 gap-5">
          <div>
            ...
            <input type="text" name="tagline" value="<?= htmlspecialchars($serviceIntro['tagline'] ?? '') ?>" class="w-full bg-background border border-border text-foreground px-4 py-3 rounded-sm text-sm font-medium">
          </div>

          <div>
            <label class="block text-xs uppercase tracking-widest text-muted-foreground mb-2">Tiêu đề chính</label>
            <input type="text" name="title_main" value="<?= htmlspecialchars($serviceIntro['title_main'] ?? '') ?>" class="w-full bg-background border border-border text-foreground px-4 py-3 rounded-sm text-sm font-medium">
          </div>

          <div>
            <label class="block text-xs uppercase tracking-widest text-muted-foreground mb-2">Tiêu đề phụ (Chữ nghiêng gold)</label>
            <input type="text" name="title_sub" value="<?= htmlspecialchars($serviceIntro['title_sub'] ?? '') ?>" class="w-full bg-background border border-border text-foreground px-4 py-3 rounded-sm text-sm font-medium">
          </div>
        </div>
        ...
        <div>
          ...
          <textarea name="description" rows="3" class="w-full bg-background border border-border text-foreground p-3 rounded-sm text-sm leading-relaxed"><?= htmlspecialchars($serviceIntro['description'] ?? '') ?></textarea>
        </div>
        ...
        <div>
          ...
          <textarea name="highlight_note" rows="2" class="w-full bg-background border border-border text-foreground p-3 rounded-sm text-sm leading-relaxed"><?= htmlspecialchars($serviceIntro['highlight_note'] ?? '') ?></textarea>
        </div>

        <div class="border-t border-border/60 pt-5 space-y-4">
          ...
          <div class="grid sm:grid-cols-2 gap-5">
            <div>
              ...
              <input type="text" name="card_tag" value="<?= htmlspecialchars($serviceIntro['card_tag'] ?? '') ?>" class="w-full bg-background border border-border text-foreground px-4 py-3 rounded-sm text-sm">
            </div>

            <div>
              <label class="block text-xs uppercase tracking-widest text-muted-foreground mb-2">Tiêu đề Card</label>
              <input type="text" name="card_title" value="<?= htmlspecialchars($serviceIntro['card_title'] ?? '') ?>" class="w-full bg-background border border-border text-foreground px-4 py-3 rounded-sm text-sm">
            </div>
          </div>
          ...
          <div>
            ...
            <input type="url" name="card_image" value="<?= htmlspecialchars($serviceIntro['card_image'] ?? '') ?>" class="w-full bg-background border border-border text-foreground px-4 py-3 rounded-sm text-sm">
          </div>
        </div>
    ...
    <div class="rounded-sm border border-border bg-card p-6 sm:p-8 space-y-6">
      ...
      <div class="grid sm:grid-cols-2 gap-6">
        <?php foreach ($pairings as $p): ?>
          <div class="rounded-sm border border-border bg-background p-5 space-y-4">
            ...
            <form action="/admin/content/pairing" method="POST" class="space-y-3">
              ...
              <div>
                ...
                <input type="text" name="title" value="<?= htmlspecialchars($p['title']) ?>" class="w-full bg-card border border-border text-foreground px-3 py-2 rounded-sm text-xs font-medium">
              </div>

              <div>
                <label class="block text-[11px] uppercase tracking-widest text-muted-foreground mb-1">Phân cấp (Standard / Premium Level)</label>
                <input type="text" name="level" value="<?= htmlspecialchars($p['level']) ?>" class="w-full bg-card border border-border text-foreground px-3 py-2 rounded-sm text-xs">
              </div>

              <div>
                <label class="block text-[11px] uppercase tracking-widest text-muted-foreground mb-1">Mô tả tóm tắt</label>
                <textarea name="subtitle" rows="2" class="w-full bg-card border border-border text-foreground p-2.5 rounded-sm text-xs leading-relaxed"><?= htmlspecialchars($p['subtitle']) ?></textarea>
              </div>

              <div class="grid grid-cols-2 gap-3">
                <div>
                  <label class="block text-[11px] uppercase tracking-widest text-muted-foreground mb-1">Mức giá khởi điểm</label>
                  <input type="text" name="price_text" value="<?= htmlspecialchars($p['price_text']) ?>" class="w-full bg-card border border-border text-foreground px-3 py-2 rounded-sm text-xs">
                </div>

                <div>
                  <label class="block text-[11px] uppercase tracking-widest text-muted-foreground mb-1">Ảnh đại diện URL</label>
                  <input type="url" name="image" value="<?= htmlspecialchars($p['image']) ?>" class="w-full bg-card border border-border text-foreground px-3 py-2 rounded-sm text-xs">
                </div>
              </div>
