...
<div class="space-y-8 max-w-4xl">
  ...
  <div class="rounded-sm border border-border bg-card p-6 sm:p-8 space-y-6">
    ...
    <form action="/admin/google-sheets/update" method="POST" class="space-y-6">
      <div>
        ...
        <input type="text" name="sheet_id" value="<?= htmlspecialchars($config['sheet_id'] ?? '') ?>" required placeholder="Ví dụ: 1BxiMVs0XRA5nFMdKvBdBZjgmUUqptlbs74OgvE2upms" class="w-full bg-background border border-border text-foreground px-4 py-3 rounded-sm text-sm font-mono">
      ...
      <div>
        ...
        <input type="url" name="webhook_url" value="<?= htmlspecialchars($config['webhook_url'] ?? '') ?>" placeholder="https://script.google.com/macros/s/AKfycbx.../exec" class="w-full bg-background border border-border text-foreground px-4 py-3 rounded-sm text-sm font-mono">
      ...
      <div class="grid sm:grid-cols-2 gap-5 pt-2">
        <label class="flex items-center gap-3 cursor-pointer p-4 rounded-sm border border-border bg-background">
      ...
      <div class="pt-4 flex flex-wrap gap-4 items-center justify-between border-t border-border">
        <button type="submit" class="btn-wine px-6 py-3.5 rounded-sm text-xs uppercase tracking-widest font-medium">
  ...
  <div class="rounded-sm border border-border bg-card p-6 sm:p-8 space-y-4">
    ...
    <pre class="bg-background p-4 rounded-sm border border-border text-xs text-foreground/80 font-mono overflow-x-auto selection:bg-[var(--wine)] select-all"><code>function doPost(e) {
  try {
